...
async function canAttemptQuiz(customerId, testId) {
    const attempts = await QuizResult.countAttempts(customerId, testId);
    return attempts < 3; // Allow up to 3 attempts
}

async function getHighestScore(customerId, testId) {
    const highestScore = await QuizResult.getHighestScore(customerId, testId);
...
app.post('/userQuiz/:testID', async (req, res) => {
    try {
        ...
        const allowed = await canAttemptQuiz(customerID, testID);
        if (!allowed) {
            console.log('Maximum number of attempts reached.');
            return res.status(403).send('You have reached the maximum number of attempts for this quiz.');
        }
        ...
        const highestScore = await QuizResult.highestScore(customerID, testID);

        // Render the result page with the score and highest score
        res.render('quizResult', { totalScore, highestScore });
        console.log('Quiz completed by customer:', customerID);
